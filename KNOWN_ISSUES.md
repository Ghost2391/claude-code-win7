...
### ~~Windows 下内容堆叠~~ 已修复
- **现象**：在 Windows 终端 (cmd, PowerShell, ConEmu, IDEA/PyCharm) 中，`/plugin`、`/mcp` 等带选择交互的界面会出现内容逐帧叠加不消失
- **原因**：Windows Console Host (conhost.exe) 和 JediTerm 处理 CSI A/B（光标上/下移）时邻近视口边界会漂移，导致 Ink 虚拟光标模型与实际位置不同步
- **修复**：
  - `packages/@ant/ink/src/core/log-update.ts` — 跨行光标移动作 `cursorMoveTo()` 时，win32 下使用绝对 CUP（`CSI row;col H`）替代 `\r` + CSI A/B/C/D 相对移动
  - `packages/@ant/ink/src/core/ink.tsx` — ConEmu (Cmder) 和 JetBrains JediTerm (IDEA/PyCharm) 下使用 `ERASE_SCREEN + CURSOR_HOME` 作为加固
- **检测逻辑**：
  - 原生 cmd/PowerShell → CUP（无闪烁）
  - ConEmu → ERASE_SCREEN（`ConEmuANSI`/`ConEmuPID` 环境变量）
  - IDEA/PyCharm → ERASE_SCREEN（`TERMINAL_EMULATOR` 含 `JetBrains`）
- **副作用**：ConEmu/JediTerm 场景有轻微闪烁（因不支持 DEC 2026 同步更新）
- **回退**：设置 `TERM_PROGRAM=anythingElse` 绕过 ConEmu 检测；IDE 终端不受此影响
